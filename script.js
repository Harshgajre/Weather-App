...
search.addEventListener('click', () => {
    ...
    const APIKey = '8a3b585171838f95a9c10460e3ea51e4';
    ...
    fetch(`https://api.openweathermap.org/data/2.5/weather?q=${city}&units=metric&appid=${APIKey}`).then(response => response.json()).then(json => {
        ...
        switch(json.weather[0].main){
            case 'Clear':
                image.src = 'weather/clear.jpg';
                break;
            case 'Rain':
                image.src = 'weather/rain.jpg';
                break;
            case 'Snow':
        ...
        if(totalCloneInfoWeather > 0){
            cloneInfoWeatherFirstclassList.remove('active-clone');
            cloneInfoHumidityFirstclassList.remove('active-clone');
            cloneInfoWindFirstclassList.remove('active-clone');
